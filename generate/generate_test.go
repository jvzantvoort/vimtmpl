...
func TestRenderExecutesTemplateAgainstConfig(t *testing.T) {
	dir := t.TempDir()
	if err := os.WriteFile(filepath.Join(dir, "bash.gtmpl"), []byte("#!/bin/bash\necho {{.ScriptName}}\n"), 0644); err != nil {
		t.Fatalf("failed to write template fixture: %v", err)
	}

	templatesDir := filepath.Join(dir, ".templates.d")
	if err := os.MkdirAll(filepath.Join(templatesDir, "default"), 0755); err != nil {
		t.Fatalf("failed to create templates dir: %v", err)
	}
	if err := os.Rename(filepath.Join(dir, "bash.gtmpl"), filepath.Join(templatesDir, "default", "bash.gtmpl")); err != nil {
		t.Fatalf("failed to move template fixture: %v", err)
	}

	t.Setenv("HOME", dir)

	cfg := &config.TemplateConfig{Lang: "bash", ScriptName: "deploy.sh"}
...
func TestRenderUnknownLang(t *testing.T) {
	...
	cfg := &config.TemplateConfig{Lang: "doesnotexist"}
	if _, err := Render(cfg); err == nil {
		t.Errorf("expected error for unknown template language")
	}
}
